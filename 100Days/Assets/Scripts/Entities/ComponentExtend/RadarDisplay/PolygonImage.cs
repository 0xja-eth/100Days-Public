...
public class PolygonImage : MaskableGraphic, ISerializationCallbackReceiver, ICanvasRaycastFilter {
    ...
    public void setWeight(int index, float value) {
        if (index < edgeWeights.Weights.Count)
            edgeWeights.Weights[index] = value;
        SetAllDirty();
    }
    ...
    public List<float> getWeights() {
        return edgeWeights.Weights;
    }
    public float getWeight(int index) {
        return edgeWeights.Weights[index];
    }
    public int getWeightCount() {
        return edgeWeights.Weights.Count;
    }

    public void setWeightCount(int count) {
        if (count > edgeWeights.Weights.Count)
            for (int i = edgeWeights.Weights.Count; i < count; i++)
                edgeWeights.Weights.Add(0);
        if (count < edgeWeights.Weights.Count)
            edgeWeights.Weights.RemoveRange(
                count, edgeWeights.Weights.Count - count);
    }
